...
async function main(a) {
    ...
    await (await NameWrapper.setSubnodeOwnerAndWrap(namehash(`${name}.ftm`), 'sub1', firstAddress, CAN_DO_EVERYTHING)).wait()
    await (await NameWrapper.setSubnodeOwnerAndWrap(namehash(`${name}.ftm`), 'sub2', firstAddress, CAN_DO_EVERYTHING)).wait()
    await (await NameWrapper.setResolver(namehash(`sub2.${name}.ftm`), resolverAddress)).wait()
    await (await Resolver.setText(namehash(`sub2.${name}.ftm`), 'domains.fnx.nft.image', 'https://i.imgur.com/JcZESMp.png')).wait()
    console.log(`Wrapped NFT for sub2.${name}.ftm is available at ${getOpenSeaUrl(NameWrapper.address, namehash(`sub2.${name}.ftm`))}`)
    await (await NameWrapper.burnFuses(namehash(`sub2.${name}.ftm`),CANNOT_UNWRAP)).wait()
    await (await NameWrapper.burnFuses(namehash(`sub2.${name}.ftm`),CANNOT_SET_RESOLVER)).wait()
    await (await NameWrapper.unwrap(namehash(`${name}.ftm`), labelhash('sub1'), firstAddress)).wait()
  }
